...
function initCardClicks() {
  ...
  if (!itemImgInput.dataset.init) {
    itemImgInput.addEventListener('change', event => {
      ...
      reader.onload = ev => { imgTag.src = ev.target.result; };
  ...
  showcaseEl.addEventListener("click", e => {
    ...
    if (imageEl) {
      e.stopPropagation();
      activeCard = imageEl.closest(".card");
      itemImgInput.click();
      return;
    }
    ...
    if (share) {
      ...
      if (!url || url === "#") { alert("リンクが設定されていません"); return; }

      const name = card.querySelector(".card-name")?.textContent || "おすすめアイテム";
      if (navigator.share) {
        navigator.share({ title: name, text: name, url }).catch(err => console.log("共有キャンセル", err));
      } else {
    ...
    if (linkEl) {
      ...
      const cards = Array.from(showcaseEl.querySelectorAll(".card"));
      const index = cards.indexOf(card);
      if (items[index]) items[index].clicks = current;

      if (linkEl.href && linkEl.href !== "#") setTimeout(() => window.open(linkEl.href, "_blank"), 150);
      return;
    }
  ...
  try {
    ...
    const state = {
      ...
      fontColor: (() => {
        ...
        return profile ? getComputedStyle(profile).color : "";
      })(),
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (commentClose && commentModal) {
    commentClose.addEventListener("click", e => { e.stopPropagation(); commentModal.style.display = "none"; });
  ...
  document.getElementById("commentSendBtn")?.addEventListener("click", () => {
    ...
    if (!text || currentCommentIndex === null) return;
    const user = document.getElementById("profileName")?.textContent || "ユーザー";
    if (!items[currentCommentIndex].comments) items[currentCommentIndex].comments = [];
    items[currentCommentIndex].comments.push({ user, text, likes: 0 });
    input.value = "";
